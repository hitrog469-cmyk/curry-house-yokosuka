...
export default function QRCodesPage() {
  ...
  const [tokens, setTokens] = useState<{ [key: number]: string }>({})
  const [tokensLoading, setTokensLoading] = useState(true)
  const totalTables = 18

  // Tokens are stored server-side (not exposed to the public). Load them via the admin API.
  useEffect(() => {
    fetch('/api/admin/qr-tokens')
      ...
      .then(data => {
        if (Array.isArray(data.tokens)) {
          ...
          for (const t of data.tokens) map[t.table_number] = t.token
          setTokens(map)
        }
      })
      .catch(() => {})
  ...
  const generateQRCodeUrl = (tableNumber: number) => {
    const token = tokens[tableNumber]
    if (!token) return ''
    // Include the secure server-validated token in the QR URL
      ...
      <body>
        <div class="container">
          ...
          <div class="instructions">
            ...
            <strong>Note:</strong> Sign-in is required.<br>
            You'll get a key after your first order — save it to add more items.
          </div>
        </div>
      </body>
      </html>
            ...
            <div key={tableNumber} className="bg-white rounded-xl shadow-md overflow-hidden hover-lift">
              ...
              <div className="p-6">
                ...
                <div className="text-xs text-gray-500 mb-4 font-mono break-all bg-gray-50 p-2 rounded">
                  {tokensLoading ? 'Loading token…' : `${baseUrl}/table-order?table=${tableNumber}&token=${tokens[tableNumber] || '(no token)'}`}
                </div>
